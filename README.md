...
一个基于 React、TypeScript 和 Vite 构建的政务风格大屏可视化项目，适用于政务服务大厅、综合业务大厅、自助终端运营监控等场景。

项目当前以内置 Mock 数据驱动，围绕“来访分布、业务统计、窗口服务、终端使用、预约情况”等核心信息构建三栏式可视化界面。页面结构已经按异步数据流组织，便于后续替换为真实业务接口。
...
- 当前仓库重点在视觉呈现、信息编排和大屏适配
- 生产环境可继续补充权限鉴权、错误兜底、加载态、监控埋点和测试用例
